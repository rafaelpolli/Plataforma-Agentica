...
function TypeBadge({ dataType }: { dataType: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-gray-100 text-xs font-mono text-gray-700">
...
function CompatChip({ item }: { item: CompatiblePort }) {
  return (
    <span className="inline-flex items-center gap-1 bg-gray-100 border border-gray-200 rounded-full px-2 py-0.5 text-xs text-gray-600 whitespace-nowrap">
      <span>{item.nodeIcon}</span>
      <span className="font-medium">{item.nodeLabel}</span>
      <span className="text-gray-400 font-light">{item.portName}</span>
    </span>
  );
}
  ...
  return (
    <div className="py-3 border-b border-gray-200 last:border-b-0">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-gray-900 text-sm font-semibold">{port.name}</span>
        {direction === 'input' && port.required && (
          <span className="text-red-500 text-xs bg-red-50 px-1.5 py-0.5 rounded">required</span>
        )}
      </div>

      <p className="text-xs text-gray-400 mb-2">{verb}:</p>
      ...
      {isAny ? (
        ...
        <p className="text-xs text-gray-300 italic">No compatible nodes found.</p>
      ) : (
...
export function NodeHelpModal({ node, onClose }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Panel */}
      <div
        className="relative bg-white border border-gray-200 rounded-2xl shadow-2xl flex flex-col"
        style={{ width: 540, maxHeight: '80vh' }}
      >
        {/* Header */}
        <div className="flex items-start gap-3 p-5 border-b border-gray-200 flex-shrink-0">
          <span className="text-3xl leading-none mt-0.5">{def.icon}</span>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h2 className="text-gray-900 font-bold text-base">{def.label}</h2>
              <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full font-mono">
                {def.category}
              </span>
            </div>
            <p className="text-gray-500 text-sm leading-relaxed">{def.description}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1">
          ...
          {node.ports.inputs.length === 0 && node.ports.outputs.length === 0 && (
            <p className="px-5 py-6 text-gray-300 text-sm text-center">This node has no ports.</p>
          )}
        </div>
      </div>
    </div>,
