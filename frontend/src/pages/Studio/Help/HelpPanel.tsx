...
export function HelpPanel({ onClose }: Props) {
  ...
  return createPortal(
    ...
    >
      <div
        className="bg-white border border-gray-200 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden"
      ...
      >
        <header className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-gray-900 font-semibold text-lg leading-tight">Agents Studio Help</h2>
              <p className="text-xs text-gray-500">Visual designer for AgentCore-native generative AI agents</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-1 overflow-hidden">
          <nav className="w-56 flex-shrink-0 bg-gray-50 border-r border-gray-200 py-3 overflow-y-auto">
            {TABS.map((t) => (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
                className={`w-full text-left px-4 py-2 text-sm flex items-center gap-2 transition-colors ${
                  tab === t.id
                    ? 'bg-orange-50 text-brand border-l-2 border-brand'
            ...
            <div className="mt-6 px-4 py-3 text-xs text-gray-400 border-t border-gray-200">
              <div className="font-semibold text-gray-500 mb-1">Shortcuts</div>
              <div className="flex justify-between"><span>Validate</span><kbd className="bg-gray-100 px-1.5 rounded text-gray-600">⌘/Ctrl+S</kbd></div>
              <div className="flex justify-between"><span>Help</span><kbd className="bg-gray-100 px-1.5 rounded text-gray-600">?</kbd></div>
              <div className="flex justify-between"><span>Close</span><kbd className="bg-gray-100 px-1.5 rounded text-gray-600">Esc</kbd></div>
            </div>
          </nav>
...
function H2({ children }: { children: React.ReactNode }) {
  return <h2 className="text-lg font-semibold text-brand mt-6 mb-2">{children}</h2>;
}
...
function Code({ children }: { children: React.ReactNode }) {
  return (
    <code className="bg-gray-100 text-amber-600 px-1.5 py-0.5 rounded text-xs font-mono">
      {children}
    </code>
  );
}
function Pre({ children }: { children: React.ReactNode }) {
  return (
    <pre className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-xs font-mono text-gray-700 overflow-x-auto mb-3">
      {children}
    </pre>
  );
}
    ...
    <ol className="space-y-2 mb-3 list-none pl-0">
      {items.map((item, i) => (
        <li key={i} className="flex gap-3">
          ...
          <div className="text-sm text-gray-600 pt-0.5">{item}</div>
        </li>
      ))}
    </ol>
  );
...
function Callout({ kind, children }: { kind: 'tip' | 'warn' | 'info'; children: React.ReactNode }) {
  const map = {
    tip: { cls: 'border-green-300 bg-green-50 text-green-700', icon: '💡' },
    warn: { cls: 'border-amber-300 bg-amber-50 text-amber-700', icon: '⚠️' },
    info: { cls: 'border-blue-300 bg-blue-50 text-blue-700', icon: 'ℹ️' },
  };
...
function Quickstart() {
  return (
    <>
      ...
      <H2>UI layout</H2>
      <ul className="text-sm text-gray-600 space-y-1 mb-3 list-disc pl-5">
    ...
    <>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {Object.entries(PORT_COLORS).map(([t, dot]) => (
          <span key={t} className="inline-flex items-center gap-1.5 bg-gray-100 px-2 py-1 rounded text-xs font-mono text-gray-700">
          ...
          <div className="grid grid-cols-2 gap-2">
            {CATEGORY_NODES[cat].map((t) => {
              ...
              return (
                <div
                  key={t}
                  className="border border-gray-200 rounded-lg p-3 bg-gray-50 hover:border-gray-300 transition-colors"
                >
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-base">{def.icon}</span>
                    <span className="text-sm font-semibold text-gray-900">{def.label}</span>
                    <span className="text-xs text-gray-400 font-mono ml-auto">{def.type}</span>
                  </div>
...
Embedding.vectors ──► Output.payload`}</Pre>
      ...
      <ul className="text-sm text-gray-600 space-y-3 mb-3">
        <li>
