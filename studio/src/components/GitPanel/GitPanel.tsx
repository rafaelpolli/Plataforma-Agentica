...
export function GitPanel({ onClose }: Props) {
  ...
  return createPortal(
    ...
    >
      <div
        className="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="flex items-center justify-between px-5 py-3 border-b border-gray-700 bg-gray-950">
          <div className="flex items-center gap-2">
            <span className="text-xl">🔀</span>
            <h2 className="text-white font-semibold text-base leading-tight">Git integration</h2>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-white text-lg" aria-label="Close">
            ✕
          </button>
        </header>
        ...
        <div className="flex border-b border-gray-700 bg-gray-950">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium transition-colors ${provider === 'github' ? 'text-white border-b-2 border-blue-400' : 'text-gray-400 hover:text-gray-200'}`}
          >
            GitHub
          </button>
          <button
            onClick={() => updateProvider('gitlab')}
            className={`px-4 py-2 text-sm font-medium transition-colors ${provider === 'gitlab' ? 'text-white border-b-2 border-blue-400' : 'text-gray-400 hover:text-gray-200'}`}
          >
            GitLab
          </button>
        </div>
        ...
        <div className="flex border-b border-gray-800 bg-gray-900">
          <button
            onClick={() => setTab('push')}
            className={`flex-1 px-4 py-2 text-sm transition-colors ${tab === 'push' ? 'bg-gray-800 text-white' : 'text-gray-500 hover:text-gray-300'}`}
          >
            ⬆ Push generated repo
          </button>
          <button
            onClick={() => setTab('pull')}
            className={`flex-1 px-4 py-2 text-sm transition-colors ${tab === 'pull' ? 'bg-gray-800 text-white' : 'text-gray-500 hover:text-gray-300'}`}
          >
            ⬇ Pull project.json
          </button>
        </div>
        ...
        <div className="px-5 py-3 space-y-3">
          ...
          {provider === 'gitlab' && (
            <div>
              <label className="block text-xs font-medium text-gray-300 mb-1">GitLab base URL (self-hosted only)</label>
              <input
                type="text"
                className={inputCls}
                value={cfg.baseUrl}
        ...
        {/* Tab-specific fields */}
        <div className="px-5 pb-3 space-y-3 border-t border-gray-800 pt-3">
          {tab === 'push' ? (
            <>
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Auto-created off the default branch if missing.</p>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-300 mb-1">Commit message</label>
                <input
                  type="text"
                  className={inputCls}
                  value={commitMessage}
            ...
            <>
              <div>
                <label className="block text-xs font-medium text-gray-300 mb-1">Ref (branch / tag / commit)</label>
                <input
                  type="text"
                  className={inputCls}
                  value={pullRef}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-300 mb-1">Path within repo</label>
                <input
                  type="text"
                  className={inputCls}
                  value={pullPath}
        ...
        {/* Action + status */}
        <div className="px-5 py-3 border-t border-gray-700 bg-gray-950 flex items-center justify-between gap-3">
          <div className="text-xs flex-1 min-w-0">
            {status === 'busy' && <span className="text-blue-300">Working…</span>}
            {status === 'ok' && (
              <span className="text-green-400">
                ✓ {resultMsg}{' '}
                {resultUrl && (
                  <a href={resultUrl} target="_blank" rel="noreferrer" className="underline hover:text-green-300 break-all">
                    view commit
                  </a>
                )}
              </span>
            )}
            {status === 'error' && <span className="text-red-400 break-words">⚠ {resultMsg}</span>}
          </div>

          <button
            onClick={tab === 'push' ? onPush : onPull}
            disabled={status === 'busy'}
            className="flex-shrink-0 px-4 py-1.5 text-sm bg-blue-600 hover:bg-blue-500 text-white rounded-lg disabled:opacity-50 transition-colors font-medium"
          >
            {tab === 'push' ? 'Push' : 'Pull'}
          </button>
        </div>
